...
```
日本語ラベル104件（9カテゴリ）を選ぶと英語プロンプトが組み上がる。**モデルで出力が変わる**——イラスト側はDanbooruタグ列、写実側は自然な英文。ネガティブも連動する（`gen.py` のプリセットと同一）。

- 常時見えるのは「主役」「場所」の2つだけ。残り7カテゴリは「もっと選ぶ」の中（見出しに選択済みラベルが並ぶ）
